...
public:
  ...
  ///
  ConfigurationSpaceCost &
  operator=(ConfigurationSpaceCost &&) noexcept = default;
  ...
  ///
  /// @brief Sets the weight vector on the configuration q at the terminal
  /// stage.
  /// @param[in] q_weight_terminal Weight vector on the configuration q at the
  /// terminal stage. Size must be Robot::dimv().
  ///
  ...
  /// @param[in] v_weight_terminal Weight vector on the velocity v at the
  /// terminal stage. Size must be Robot::dimv().
  ///
  ...
  /// the impact stages.
  /// @param[in] dv_weight_impact Weight vector on the impact change in the
  /// velocity the impact stages. Size must be Robot::dimv().
  ///
  ...
  void evalStageCostDerivatives(Robot &robot,
  ...
  void
  evalTerminalCostDerivatives(Robot &robot, CostFunctionData &data,
  ...
  void evalImpactCostDerivatives(Robot &robot,
...
private:
  ...
  Eigen::VectorXd q_ref_, v_ref_, u_ref_, q_weight_, v_weight_, a_weight_,
      u_weight_, q_weight_terminal_, v_weight_terminal_, q_weight_impact_,
      v_weight_impact_, dv_weight_impact_;
  std::shared_ptr<ConfigurationSpaceRefBase> ref_;
  bool use_nonconst_ref_, enable_q_cost_, enable_v_cost_, enable_a_cost_,
      enable_u_cost_, enable_q_cost_terminal_, enable_v_cost_terminal_,
