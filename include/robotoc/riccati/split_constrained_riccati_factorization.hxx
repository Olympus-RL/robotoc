...
inline SplitConstrainedRiccatiFactorization::
    ...
    : Ginv(Eigen::MatrixXd::Zero(robot.dimu(), robot.dimu())),
      ...
      DGinv_full_(
          Eigen::MatrixXd::Zero(robot.max_dimf_contact(), robot.dimu())),
      S_full_(Eigen::MatrixXd::Zero(robot.max_dimf_contact(),
                                    robot.max_dimf_contact())),
      Sinv_full_(Eigen::MatrixXd::Zero(robot.max_dimf_contact(),
                                       robot.max_dimf_contact())),
      SinvDGinv_full_(
          Eigen::MatrixXd::Zero(robot.max_dimf_contact(), robot.dimu())),
      dimv_(robot.dimv()), dimx_(2 * robot.dimv()), dimu_(robot.dimu()),
      dims_(0) {}

inline SplitConstrainedRiccatiFactorization::
    SplitConstrainedRiccatiFactorization()
    : Ginv(), DtM(), KtDtM(), DGinv_full_(), S_full_(), Sinv_full_(),
      SinvDGinv_full_(), dimv_(0), dimx_(0), dimu_(0), dims_(0) {}

inline SplitConstrainedRiccatiFactorization::
    ~SplitConstrainedRiccatiFactorization() {}

inline void
SplitConstrainedRiccatiFactorization::setConstraintDimension(const int dimi) {
  dims_ = dimi;
}

inline int SplitConstrainedRiccatiFactorization::dims() const { return dims_; }
...
inline Eigen::Block<Eigen::MatrixXd> SplitConstrainedRiccatiFactorization::S() {
...
inline bool SplitConstrainedRiccatiFactorization::isApprox(
  ...
  if (dims() != other.dims())
    return false;
  if (!DGinv().isApprox(other.DGinv()))
    return false;
  if (!S().isApprox(other.S()))
    return false;
  if (!Sinv().isApprox(other.Sinv()))
    return false;
  if (!SinvDGinv().isApprox(other.SinvDGinv()))
    return false;
  if (!Ginv.isApprox(other.Ginv))
    return false;
  if (!DtM.isApprox(other.DtM))
    return false;
  if (!KtDtM.isApprox(other.KtDtM))
    return false;
  return true;
}
