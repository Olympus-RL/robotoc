...
inline SplitRiccatiFactorization::SplitRiccatiFactorization(const Robot &robot)
    : P(Eigen::MatrixXd::Zero(2 * robot.dimv(), 2 * robot.dimv())),
      ...
      Phi(Eigen::VectorXd::Zero(2 * robot.dimv())), xi(0.0), chi(0.0), rho(0.0),
      eta(0.0), iota(0.0), M_full_(Eigen::MatrixXd::Zero(
                               robot.max_dimf_contact(), 2 * robot.dimv())),
      m_full_(Eigen::VectorXd::Zero(robot.max_dimf_contact())),
      mt_full_(Eigen::VectorXd::Zero(robot.max_dimf_contact())),
      mt_next_full_(Eigen::VectorXd::Zero(robot.max_dimf_contact())),
      dimv_(robot.dimv()), dimx_(2 * robot.dimv()), dims_(0) {}

inline SplitRiccatiFactorization::SplitRiccatiFactorization()
    : P(), s(), psi_x(), psi_u(), Psi(), phi_x(), phi_u(), Phi(), xi(0.0),
      chi(0.0), rho(0.0), eta(0.0), iota(0.0), M_full_(), m_full_(), mt_full_(),
      mt_next_full_(), dimv_(0), dimx_(0), dims_(0) {}
...
inline int SplitRiccatiFactorization::dims() const { return dims_; }
...
SplitRiccatiFactorization::M() const {
  return M_full_.topLeftCorner(dims_, dimx_);
}
...
inline const Eigen::VectorBlock<const Eigen::VectorXd>
SplitRiccatiFactorization::mt() const {
  return mt_full_.head(dims_);
}

inline Eigen::VectorBlock<Eigen::VectorXd>
SplitRiccatiFactorization::mt_next() {
  return mt_next_full_.head(dims_);
}

inline const Eigen::VectorBlock<const Eigen::VectorXd>
SplitRiccatiFactorization::mt_next() const {
  return mt_next_full_.head(dims_);
}
...
inline bool SplitRiccatiFactorization::isApprox(
  ...
  if (!P.isApprox(other.P))
    return false;
  if (!s.isApprox(other.s))
    return false;
  if (!psi_x.isApprox(other.psi_x))
    return false;
  if (!psi_u.isApprox(other.psi_u))
    return false;
  if (!Psi.isApprox(other.Psi))
    return false;
  if (!phi_x.isApprox(other.phi_x))
    return false;
  if (!phi_u.isApprox(other.phi_u))
    return false;
  if (!Phi.isApprox(other.Phi))
    return false;
  ...
  if (!vec.isApprox(other_vec))
    return false;
  if (dims() > 0) {
    if (!M().isApprox(other.M()))
      return false;
    if (!m().isApprox(other.m()))
      return false;
    if (!mt().isApprox(other.mt()))
      return false;
    if (!mt_next().isApprox(other.mt_next()))
      return false;
  }
  return true;
}
...
inline SplitRiccatiFactorization
SplitRiccatiFactorization::Random(const Robot &robot) {
