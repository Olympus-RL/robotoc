...
inline void Robot::dSubtractConfiguration_dqf(
  ...
  pinocchio::dDifference(
      model_, q0, qf, const_cast<Eigen::MatrixBase<MatrixType> &>(dqdiff_dqf),
      pinocchio::ARG1);
}
...
inline void Robot::dSubtractConfiguration_dq0(
  ...
  pinocchio::dDifference(
      model_, q0, qf, const_cast<Eigen::MatrixBase<MatrixType> &>(dqdiff_dq0),
      pinocchio::ARG0);
}
...
inline void Robot::integrateCoeffWiseJacobian(
  ...
  pinocchio::integrateCoeffWiseJacobian(
      model_, q, const_cast<Eigen::MatrixBase<MatrixType> &>(J));
}

template <typename ConfigVectorType, typename TangentVectorType1,
          typename TangentVectorType2>
inline void
Robot::updateKinematics(const Eigen::MatrixBase<ConfigVectorType> &q,
...
template <typename ConfigVectorType, typename TangentVectorType1,
          typename TangentVectorType2>
inline void
Robot::updateFrameKinematics(const Eigen::MatrixBase<ConfigVectorType> &q,
                             const Eigen::MatrixBase<TangentVectorType1> &v,
                             const Eigen::MatrixBase<TangentVectorType2> &a,
...
template <typename MatrixType>
inline void
Robot::getFrameWorldJacobian(const std::string frame_name,
                             const Eigen::MatrixBase<MatrixType> &J) {
  assert(J.rows() == 6);
  assert(J.cols() == dimv_);
  assert(model_.existFrame(frame_name)),
      std::invalid_argument("Error: frame " + frame_name +
...
template <typename MatrixType>
inline void
Robot::getCoMJacobian(const Eigen::MatrixBase<MatrixType> &J) const {
  assert(J.rows() == 3);
  assert(J.cols() == dimv_);
...
inline void
Robot::RNEADerivatives(const Eigen::MatrixBase<ConfigVectorType> &q,
                       const Eigen::MatrixBase<TangentVectorType1> &v,
                       const Eigen::MatrixBase<TangentVectorType2> &a,
...
template <typename ConfigVectorType, typename TangentVectorType1,
          typename TangentVectorType2>
inline void
Robot::RNEAImpact(const Eigen::MatrixBase<ConfigVectorType> &q,
...
inline void Robot::RNEAImpactDerivatives(
  ...
  pinocchio::computeRNEADerivatives(
      impact_model_, impact_data_, q, Eigen::VectorXd::Zero(dimv_), dv, fjoint_,
      const_cast<Eigen::MatrixBase<MatrixType1> &>(dRNEA_partial_dq),
      dimpact_dv_,
...
template <typename MatrixType1, typename MatrixType2, typename MatrixType3>
inline void
Robot::computeMJtJinv(const Eigen::MatrixBase<MatrixType1> &M,
  ...
  Eigen::Block<MatrixType3> topLeft =
      const_cast<Eigen::MatrixBase<MatrixType3> &>(MJtJinv).topLeftCorner(
          dimv_, dimv_);
  Eigen::Block<MatrixType3> topRight =
      const_cast<Eigen::MatrixBase<MatrixType3> &>(MJtJinv).topRightCorner(
          dimv_, dimf);
  Eigen::Block<MatrixType3> bottomLeft =
      const_cast<Eigen::MatrixBase<MatrixType3> &>(MJtJinv).bottomLeftCorner(
          dimf, dimv_);
  Eigen::Block<MatrixType3> bottomRight =
      const_cast<Eigen::MatrixBase<MatrixType3> &>(MJtJinv).bottomRightCorner(
          dimf, dimf);
