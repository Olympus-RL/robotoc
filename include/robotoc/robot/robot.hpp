...
public:
  ...
  void
  integrateConfiguration(const Eigen::MatrixBase<TangentVectorType> &v,
  ...
  void dIntegrateTransport_dq(const Eigen::MatrixBase<ConfigVectorType> &q,
                              const Eigen::MatrixBase<TangentVectorType> &v,
  ...
  template <typename ConfigVectorType, typename TangentVectorType,
            typename MatrixType1, typename MatrixType2>
  void dIntegrateTransport_dv(const Eigen::MatrixBase<ConfigVectorType> &q,
                              const Eigen::MatrixBase<TangentVectorType> &v,
                              const Eigen::MatrixBase<MatrixType1> &Jin,
                              const Eigen::MatrixBase<MatrixType2> &Jout) const;
  ...
  ///
  /// @brief Returns the linear (translational) velocity of the frame. Before
  /// calling this function, updateKinematics() or updateFrameKinematics() must
  /// be called.
  /// @param[in] frame_id Index of the frame.
  ...
  ///
  Eigen::Vector3d
  frameLinearVelocity(const int frame_id,
  ...
  ///
  Eigen::Vector3d
  frameLinearVelocity(const std::string &frame_name,
                      const pinocchio::ReferenceFrame reference_frame =
                          pinocchio::LOCAL_WORLD_ALIGNED) const;

  ///
  /// @brief Returns the angular velocity of the frame. Before calling
  /// this function, updateKinematics() or updateFrameKinematics() must be
  /// called.
  /// @param[in] frame_id Index of the frame.
  /// @param[in] reference_frame Reference frame. Default is
  /// pinocchio::LOCAL_WORLD_ALIGNED (world velocity).
  ...
  ///
  Eigen::Vector3d
  frameAngularVelocity(const int frame_id,
                       const pinocchio::ReferenceFrame reference_frame =
                           pinocchio::LOCAL_WORLD_ALIGNED) const;
  ...
  ///
  Eigen::Vector3d
  frameAngularVelocity(const std::string &frame_name,
                       const pinocchio::ReferenceFrame reference_frame =
                           pinocchio::LOCAL_WORLD_ALIGNED) const;
  ...
  ///
  Vector6d
  frameSpatialVelocity(const int frame_id,
                       const pinocchio::ReferenceFrame reference_frame =
                           pinocchio::LOCAL_WORLD_ALIGNED) const;
  ...
  ///
  Vector6d
  frameSpatialVelocity(const std::string &frame_name,
                       const pinocchio::ReferenceFrame reference_frame =
                           pinocchio::LOCAL_WORLD_ALIGNED) const;
  ...
  /// @param[in] impact_status Impact status.
  /// @param[out] position_residual Residuals in the contact position
  /// constraint. Size must be ImpactStatus::dimf().
  ///
  template <typename VectorType>
  ...
  ///
  /// @brief Computes the inverse of the contact dynamics matrix [[M J^T], [J
  /// O]].
  /// @param[in] M Joint inertia matrix. Size must be
  /// Robot::dimv() x Robot::dimv().
  ...
  /// @param[in, out] q The normalized configuration. Size must be
  /// Robot::dimq().
  ///
  template <typename ConfigVectorType>
  void
  normalizeConfiguration(const Eigen::MatrixBase<ConfigVectorType> &q) const;
  ...
  ///
  /// @brief Initializes the results of jointEffortLimit(),
  /// jointVelocityLimit(), lowerJointPositionLimit(), and
  /// upperJointPositionLimit() by the URDF.
  ///
  ...
  ///
  void
  setLowerJointPositionLimit(const Eigen::VectorXd &lower_joint_position_limit);
  ...
  ///
  void
  setUpperJointPositionLimit(const Eigen::VectorXd &upper_joint_position_limit);
  ...
  /// @return const reference to a collectio of the properties of this robot
  /// model.
  ///
