...
public:
  ...
  ///
  /// @brief CKC constraint forces. Upper 2 elements are the constraint force in
  /// "motor housing frame" Size is Robot::numCKC().
  ///
  ...
  ///
  Eigen::VectorBlock<Eigen::VectorXd> gf_stack();

  ///
  /// @brief Const version of SplitSolution::f_stack().
  ///
  const Eigen::VectorBlock<const Eigen::VectorXd> gf_stack() const;
  ...
  ///
  /// @brief Lagrange multiplier w.r.t. the acceleration-level CKC
  /// constraint.
  ///
  std::vector<Vector2d> rho;
  ...
  ///
  /// @brief Returns the dimension of the ckc forces
  /// @return Dimension of the contact.
  ///
  int dimg() const;
...
private:
  Eigen::VectorXd rhomu_stack_, gf_stack_, xi_stack_;
  bool has_floating_base_;
  std::vector<ContactType> contact_types_;
  std::vector<bool> is_contact_active_;
  int dimf_, dimg_, dims_, max_num_contacts_, num_ckcs_;
};
