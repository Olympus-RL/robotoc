...
protected:
  virtual void SetUp() {
    ...
    cone.resize(5, 3);
    cone.setZero();
    cone << 0, 0, -1, 1, 0, -(mu / std::sqrt(2)), -1, 0, -(mu / std::sqrt(2)),
        0, 1, -(mu / std::sqrt(2)), 0, -1, -(mu / std::sqrt(2));
  ...
  void test_evalDerivatives(Robot &robot,
                            const ImpactStatus &impact_status) const;
  void test_evalConstraint(Robot &robot,
                           const ImpactStatus &impact_status) const;
  void test_condenseSlackAndDual(Robot &robot,
                                 const ImpactStatus &impact_status) const;
  void test_expandSlackAndDual(Robot &robot,
                               const ImpactStatus &impact_status) const;
...
void ImpactFrictionConeTest::test_kinematics(
    Robot &robot, const ImpactStatus &impact_status) const {
  ImpactFrictionCone constr(robot);
  EXPECT_TRUE(constr.kinematicsLevel() == KinematicsLevel::AccelerationLevel);
}

void ImpactFrictionConeTest::test_isFeasible(
    Robot &robot, const ImpactStatus &impact_status) const {
  ImpactFrictionCone constr(robot);
  ...
  if (impact_status.hasActiveImpact()) {
    ...
    for (int i = 0; i < impact_status.maxNumContacts(); ++i) {
      if (impact_status.isImpactActive(i)) {
        ...
        robot.transformFromLocalToWorld(robot.contactFrames()[i],
                                        s.f[i].template head<3>(), f_world);
        Eigen::VectorXd res = Eigen::VectorXd::Zero(5);
        ImpactFrictionCone::frictionConeResidual(mu, f_world, contact_surface,
                                                 res);
        if (res.maxCoeff() > 0) {
          feasible = false;
        }
    ...
    EXPECT_EQ(constr.isFeasible(robot, impact_status, data, s), feasible);
  }
}

void ImpactFrictionConeTest::test_setSlack(
    Robot &robot, const ImpactStatus &impact_status) const {
  ImpactFrictionCone constr(robot);
  ConstraintComponentData data(constr.dimc(), constr.getBarrierParam()),
      data_ref(constr.dimc(), constr.getBarrierParam());
  constr.allocateExtraData(data);
  constr.allocateExtraData(data_ref);
  const int dimc = constr.dimc();
  const auto s = SplitSolution::Random(robot, impact_status);
  robot.updateFrameKinematics(s.q);
  constr.setSlack(robot, impact_status, data, s);
  for (int i = 0; i < impact_status.maxNumContacts(); ++i) {
    Eigen::Vector3d f_world = Eigen::Vector3d::Zero();
    robot.transformFromLocalToWorld(robot.contactFrames()[i],
                                    s.f[i].template head<3>(), f_world);
    ImpactFrictionCone::frictionConeResidual(
        mu, f_world, contact_surface, data_ref.residual.segment(5 * i, 5));
    data_ref.slack.segment(5 * i, 5) = -data_ref.residual.segment(5 * i, 5);
  }
  EXPECT_TRUE(data.isApprox(data_ref));
}

void ImpactFrictionConeTest::test_evalConstraint(
    Robot &robot, const ImpactStatus &impact_status) const {
  ImpactFrictionCone constr(robot);
  const int dimc = constr.dimc();
  const auto s = SplitSolution::Random(robot, impact_status);
  ...
  data.cmpl.setRandom();
  auto data_ref = data;
  constr.evalConstraint(robot, impact_status, data, s);
  ...
  for (int i = 0; i < impact_status.maxNumContacts(); ++i) {
    if (impact_status.isImpactActive(i)) {
      ...
      ImpactFrictionCone::frictionConeResidual(
          mu, f_world, contact_surface, data_ref.residual.segment(5 * i, 5));
      data_ref.residual.template segment<5>(5 * i) +=
          data_ref.slack.segment(5 * i, 5);
      for (int j = 0; j < 5; ++j) {
        data_ref.cmpl.coeffRef(5 * i + j) =
            data_ref.slack.coeff(5 * i + j) * data_ref.dual.coeff(5 * i + j) -
            barrier_param;
      }
      data_ref.log_barrier +=
          pdipm::logBarrier(barrier_param, data_ref.slack.segment(5 * i, 5));
    }
  }
  EXPECT_TRUE(data.isApprox(data_ref));
}

void ImpactFrictionConeTest::test_evalDerivatives(
    Robot &robot, const ImpactStatus &impact_status) const {
  ImpactFrictionCone constr(robot);
  ConstraintComponentData data(constr.dimc(), constr.getBarrierParam());
  constr.allocateExtraData(data);
  ...
  constr.evalDerivatives(robot, impact_status, data, s, kkt_res);
  int dimf_stack = 0;
  for (int i = 0; i < impact_status.maxNumContacts(); ++i) {
    if (impact_status.isImpactActive(i)) {
      Eigen::Vector3d f_world = Eigen::Vector3d::Zero();
      ...
      const Eigen::MatrixXd dg_df =
          cone_surface_local * robot.frameRotation(robot.contactFrames()[i]);
...
void ImpactFrictionConeTest::test_condenseSlackAndDual(
    Robot &robot, const ImpactStatus &impact_status) const {
  ImpactFrictionCone constr(robot);
  ConstraintComponentData data(constr.dimc(), constr.getBarrierParam());
  constr.allocateExtraData(data);
  ...
  auto kkt_res_ref = kkt_res;
  constr.condenseSlackAndDual(impact_status, data, kkt_mat, kkt_res);
  int dimf_stack = 0;
  for (int i = 0; i < impact_status.maxNumContacts(); ++i) {
    if (impact_status.isImpactActive(i)) {
      Eigen::Vector3d f_world = Eigen::Vector3d::Zero();
      ...
      r.array() = (data_ref.dual.segment(5 * i, 5).array() *
                       data_ref.residual.segment(5 * i, 5).array() -
                   data_ref.cmpl.segment(5 * i, 5).array()) /
      ...
      r.array() = data_ref.dual.segment(5 * i, 5).array() /
                  data_ref.slack.segment(5 * i, 5).array();
      kkt_mat_ref.Qqq() += dg_dq.transpose() * r.asDiagonal() * dg_dq;
...
void ImpactFrictionConeTest::test_expandSlackAndDual(
    Robot &robot, const ImpactStatus &impact_status) const {
  ImpactFrictionCone constr(robot);
  ConstraintComponentData data(constr.dimc(), constr.getBarrierParam());
  constr.allocateExtraData(data);
  ...
  constr.expandSlackAndDual(impact_status, data, d);
  ...
  for (int i = 0; i < impact_status.maxNumContacts(); ++i) {
    if (impact_status.isImpactActive(i)) {
      ...
      data_ref.dslack.segment(5 * i, 5) =
          -dg_dq * d.dq() - dg_df * d.df_contact().segment(dimf_stack, 3) -
          data_ref.residual.segment(5 * i, 5);
      for (int j = 0; j < 5; ++j) {
        data_ref.ddual(5 * i + j) =
            -(data_ref.dual(5 * i + j) * data_ref.dslack(5 * i + j) +
              data_ref.cmpl(5 * i + j)) /
