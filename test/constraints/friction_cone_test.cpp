...
void FrictionConeTest::test_kinematics(
    Robot &robot, const ContactStatus &contact_status) const {
  FrictionCone constr(robot);
  EXPECT_TRUE(constr.kinematicsLevel() == KinematicsLevel::AccelerationLevel);
}

void FrictionConeTest::test_isFeasible(
    Robot &robot, const ContactStatus &contact_status) const {
  FrictionCone constr(robot);
    ...
    for (int i = 0; i < contact_status.maxNumContacts(); ++i) {
      if (contact_status.isContactActive(i)) {
        ...
        robot.transformFromLocalToWorld(robot.contactFrames()[i],
                                        s.f_contact[i].template head<3>(), f_world);
...
void FrictionConeTest::test_setSlack(
    Robot &robot, const ContactStatus &contact_status) const {
  FrictionCone constr(robot);
  ConstraintComponentData data(constr.dimc(), constr.getBarrierParam()),
  ...
  for (int i = 0; i < contact_status.maxNumContacts(); ++i) {
    ...
    robot.transformFromLocalToWorld(robot.contactFrames()[i],
                                    s.f_contact[i].template head<3>(), f_world);
    FrictionCone::frictionConeResidual(mu, f_world, contact_surface,
...
void FrictionConeTest::test_evalConstraint(
    Robot &robot, const ContactStatus &contact_status) const {
  FrictionCone constr(robot);
  const int dimc = constr.dimc();
  const auto s = SplitSolution::Random(robot, contact_status);
...
void FrictionConeTest::test_evalDerivatives(
    Robot &robot, const ContactStatus &contact_status) const {
  FrictionCone constr(robot);
  ConstraintComponentData data(constr.dimc(), constr.getBarrierParam());
  constr.allocateExtraData(data);
...
void FrictionConeTest::test_condenseSlackAndDual(
    Robot &robot, const ContactStatus &contact_status) const {
  FrictionCone constr(robot);
  ConstraintComponentData data(constr.dimc(), constr.getBarrierParam());
  constr.allocateExtraData(data);
...
void FrictionConeTest::test_expandSlackAndDual(
    Robot &robot, const ContactStatus &contact_status) const {
  FrictionCone constr(robot);
  ConstraintComponentData data(constr.dimc(), constr.getBarrierParam());
  constr.allocateExtraData(data);
