...
protected:
  virtual void SetUp() {
    ...
    cone.resize(17, 6);
    cone.setZero();
    cone << 0, 0, -1, 0, 0, 0, -1, 0, -mu, 0, 0, 0, 1, 0, -mu, 0, 0, 0, 0, -1,
        -mu, 0, 0, 0, 0, 1, -mu, 0, 0, 0, 0, 0, -Y, -1, 0, 0, 0, 0, -Y, 1, 0, 0,
        0, 0, -X, 0, -1, 0, 0, 0, -X, 0, 1, 0, -Y, -X, -(X + Y) * mu, mu, mu,
        -1, -Y, X, -(X + Y) * mu, mu, -mu, -1, Y, -X, -(X + Y) * mu, -mu, mu,
        -1, Y, X, -(X + Y) * mu, -mu, -mu, -1, Y, X, -(X + Y) * mu, mu, mu, 1,
        Y, -X, -(X + Y) * mu, mu, -mu, 1, -Y, X, -(X + Y) * mu, -mu, mu, 1, -Y,
        -X, -(X + Y) * mu, -mu, -mu, 1;
  }
...
void ImpactWrenchConeTest::test_isFeasible(
...
void ImpactWrenchConeTest::test_setSlack(
    Robot &robot, const ImpactStatus &impact_status) const {
  ImpactWrenchCone constr(robot, X, Y);
  ConstraintComponentData data(constr.dimc(), constr.getBarrierParam()),
  ...
  for (int i = 0; i < robot.maxNumContacts(); ++i) {
    ...
    case ContactType::SurfaceContact:
      if (impact_status.isImpactActive(i)) {
        ...
        data_ref.slack.segment(c_begin, 17) =
...
void ImpactWrenchConeTest::test_evalConstraint(
    Robot &robot, const ImpactStatus &impact_status) const {
  ImpactWrenchCone constr(robot, X, Y);
  const int dimc = constr.dimc();
  const auto s = SplitSolution::Random(robot, impact_status);
  ...
  for (int i = 0; i < impact_status.maxNumContacts(); ++i) {
    ...
    case ContactType::SurfaceContact:
      if (impact_status.isImpactActive(i)) {
        ...
        pdipm::computeComplementarySlackness(barrier_param, data_ref, c_begin,
                                             17);
        data_ref.log_barrier += pdipm::logBarrier(
            barrier_param, data_ref.slack.segment(c_begin, 17));
      }
      c_begin += 17;
      break;
    default:
...
void ImpactWrenchConeTest::test_evalDerivatives(
    Robot &robot, const ImpactStatus &impact_status) const {
  ImpactWrenchCone constr(robot, X, Y);
  ConstraintComponentData data(constr.dimc(), constr.getBarrierParam());
  constr.allocateExtraData(data);
...
void ImpactWrenchConeTest::test_condenseSlackAndDual(
    Robot &robot, const ImpactStatus &impact_status) const {
  ImpactWrenchCone constr(robot, X, Y);
  ConstraintComponentData data(constr.dimc(), constr.getBarrierParam());
  constr.allocateExtraData(data);
...
void ImpactWrenchConeTest::test_expandSlackAndDual(
    Robot &robot, const ImpactStatus &impact_status) const {
  ImpactWrenchCone constr(robot, X, Y);
  ConstraintComponentData data(constr.dimc(), constr.getBarrierParam());
  constr.allocateExtraData(data);
    ...
    case ContactType::SurfaceContact:
      if (impact_status.isImpactActive(i)) {
        data_ref.dslack.segment(c_begin, 17).noalias() =
            -cone * d.df_contact().segment(dimf_stack, 6) -
            data_ref.residual.segment(c_begin, 17);
