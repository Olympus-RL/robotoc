...
dt = 0.005
init_height = 0.25
flight_duration = np.sqrt(2*init_height/3.72)
touch_down_duration = 0.5
t0 = 0.

# Create the cost function
cost = robotoc.CostFunction()
q_standing = np.array([0., 0., 0.44, 0.0, 0., 0., 1.0, 
                         0.0,  0.6,  1.0, -.6, -.0,  #back left
                        -0.0, -0.6,  1.0,  .6, -.0, #back right
                        -0.0,  0.6, -1.0, -.6,  .0, #front left
                         0.0,  0.6, -1.0,  .6,  .0]) #front right
                              ###inner###  ###outer###
...
contact_sequence.init(contact_status_flying)
...
contact_sequence.push_back(contact_status_standing, t0+flight_duration, sto=False)
...
theta = calcualate_joint_states(robot, contact_sequence.contact_status(1),pose, theta)
q[:7] = pose
q[7:] = theta
q_standing = q.copy()
...
dv_weight_impact = np.full(robot.dimv(), 1.0e-3)
...
sto_constraints = robotoc.STOConstraints(minimum_dwell_times=[0.10,0.10],
                                            barrier_param=1.0e-03, 
                                            fraction_to_boundary_rule=0.995)

T = t0 +flight_duration + touch_down_duration
N = math.floor(T/dt) 
