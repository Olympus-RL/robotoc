...
def ID_OLD(robot,q,v,a,contact_status,torque_limits) -> Tuple[NDArray,List[NDArray],List[NDArray]]:
    robot.update_kinematics(q,v,a)
    ...
    J_contact = robot.get_contact_position_jacobian(contact_status)
    J_contact_base = J_contact[:,:6]
    J_contact_joints = J_contact[:,6:]
    u_virtual = robot.rnea(q,v,a)
    u_virtual_base = u_virtual[:6]



    num_ineq = 5*num_contacts
    dim_x = dim_u + dim_f_contact
    A_ineq = np.zeros((num_ineq,dim_x))
    b_ineq = np.zeros(num_ineq)
    A_ineq = A_friction_cone
    b_ineq = b_friction_cone

    num_eq = 6
    A_eq = np.zeros((num_eq,dim_x))
    b_eq = u_virtual_base
    A_eq = J_contact_base.T

    I_slack = np.eye(num_ineq)
    c_slack = c_slack_friction_cone
    bounds_slack = [(0,None) for _ in range(num_ineq)]

    c =np.zeros(dim_f_contact)
    c = np.concatenate((c,c_slack))
    bounds =  bounds_friction_cone + bounds_slack
    ...
    f_contact_world = res.x[:dim_f_contact]
    F_contact = []
    ...
    u = u_virtual[6:] - J_contact_joints.T @ f_contact_world

    ## residuals

    res_id = np.linalg.norm(A_eq[:,:-num_ineq] @ res.x[:-num_ineq] - b_eq)
    res_ineq = (A_ineq[:,:-num_ineq] @ res.x[:-num_ineq] - b_ineq).clip(min=0)
    res_friction_cone = np.linalg.norm(res_ineq)

    if res_id + res_friction_cone  > 1e-3:
        print("Warning: residuals are large")
        print("residuals ID: ", res_id)
        print("residuals friction cone:", res_friction_cone)
