...
int main(int argc, char *argv[]) {
  ...
  model_info.urdf_path = "/home/bolivar/OLYMPOC/robotoc/descriptions/"
                         "olympus_description/urdf/olympus_open_scale_10.urdf";
  ...
  robot.setUpperJointPositionLimit(
      Eigen::VectorXd::Constant(robot.dimv() - 6, 1.57));
  robot.setJointEffortLimit(Eigen::VectorXd::Constant(robot.dimv() - 6, 25.0));
  robot.setJointVelocityLimit(
      Eigen::VectorXd::Constant(robot.dimv() - 6, 31.0));

  const double t0 = 0.0;
  const double dt = 0.02;
  const Eigen::Vector3d jump_length = {.8, 0, 0};
  const double takoff_time = 0.5;
  const double touchdown_time = 0.3;
  ...
  optJump.SetJumpLength(jump_length(0));
  optJump.SetInitialBaseState(q_standing.head(3), Eigen::Vector3d::Zero());
  towr::OptJump::EEpos ee_pos_towr;
  ee_pos_towr.push_back(x3d0_LF);
  ee_pos_towr.push_back(x3d0_RF);
  ee_pos_towr.push_back(x3d0_LH);
  ee_pos_towr.push_back(x3d0_RH);
  for (auto &ee_pos : ee_pos_towr) {
    ee_pos(2) = 0.0;
  }
  optJump.SetTakeoffDuration(takoff_time);
  optJump.SetInitialEEState(ee_pos_towr);
  optJump.Solve();
  flying_time = optJump.GetJumpDuration();
  ...
  contact_sequence->push_back(contact_status_flying, t0 + takoff_time, true);
  ...
  contact_sequence->push_back(contact_status_standing,
                              t0 + takoff_time + flying_time, true);
  ...
  const std::vector<double> minimum_dwell_times = {
      takoff_time - 0.2, flying_time * 0.5, touchdown_time * 0.8};
  ...
  const double T = t0 + takoff_time + flying_time + touchdown_time;
  ...
  robotoc::Solution solution_towr;
  robotoc::TimeDiscretization time_discretization =
      ocp_solver.getTimeDiscretization();
  solution_towr = ocp_solver.getSolution();

  int stage = 0;
  for (robotoc::SplitSolution &s : solution_towr) {
    robotoc::GridInfo grid = time_discretization[stage++];
    Eigen::VectorXd x_towr = optJump.GetBaseState(grid.t);
    s.q.head(3) = x_towr.head(3);
    s.v.head(3) = x_towr.segment(6, 3);
    std::cout << grid.t << std::endl;
  }

  ocp_solver.setSolution(solution_towr);
