...
int main(int argc, char *argv[]) {
  ...
  Eigen::VectorXd v_weight = Eigen::VectorXd::Constant(robot.dimv(), 1.0e-01);
  ...
  Eigen::VectorXd B(12);
  B.setZero();
  robot.computeBaumgarteResidual(contact_status_standing, B);
  std::cout << "Baumgarte residual: " << B.transpose() << std::endl;

  // you can check the contact sequence via
  // std::cout << contact_sequence << std::endl;

  const double T = t0 + standing_time;
