...
OCPSolver::OCPSolver(const OCP &ocp, const SolverOptions &solver_options)
    : robots_(solver_options.nthreads, ocp.robot),
      ...
      dms_(ocp, solver_options.nthreads), sto_(ocp),
      riccati_recursion_(ocp, solver_options.max_dts_riccati),
      line_search_(ocp, solver_options.line_search_settings), ocp_(ocp),
      kkt_matrix_(ocp.N + 1 + ocp.reserved_num_discrete_events,
                  SplitKKTMatrix(ocp.robot)),
      kkt_residual_(ocp.N + 1 + ocp.reserved_num_discrete_events,
                    SplitKKTResidual(ocp.robot)),
      s_(ocp.N + 1 + ocp.reserved_num_discrete_events,
         SplitSolution(ocp.robot)),
      d_(ocp.N + 1 + ocp.reserved_num_discrete_events,
         SplitDirection(ocp.robot)),
      riccati_factorization_(ocp.N + 1 + ocp.reserved_num_discrete_events + 1,
                             SplitRiccatiFactorization(ocp.robot)),
      solution_interpolator_(solver_options.interpolation_order),
      solver_options_(solver_options), solver_statistics_(), timer_() {
  if (!ocp.cost) {
    throw std::out_of_range(
        "[OCPSolver] invalid argument: ocp.cost should not be nullptr!");
  }
  if (!ocp.constraints) {
    throw std::out_of_range(
        "[OCPSolver] invalid argument: ocp.constraints should not be nullptr!");
  }
  if (!ocp.contact_sequence) {
    throw std::out_of_range("[OCPSolver] invalid argument: "
                            "ocp.contact_sequence should not be nullptr!");
  }
  if (ocp.T <= 0) {
    throw std::out_of_range(
        "[OCPSolver] invalid argument: ocp.T must be positive!");
  }
  if (ocp.N <= 0) {
    throw std::out_of_range(
        "[OCPSolver] invalid argument: ocp.N must be positive!");
  }
  if (ocp.reserved_num_discrete_events < 0) {
    throw std::out_of_range(
        "[OCPSolver] invalid argument: ocp.reserved_num_discrete_events must "
        "be non-negative!");
  }
  if (solver_options.nthreads <= 0) {
    throw std::out_of_range("[OCPSolver] invalid argument: "
                            "solver_options.nthreads must be positive!");
  }
  for (auto &e : s_) {
    ocp.robot.normalizeConfiguration(e.q);
  }
...
OCPSolver::OCPSolver()
    : robots_(), contact_sequence_(), time_discretization_(), dms_(), sto_(),
      riccati_recursion_(), line_search_(), ocp_(), kkt_matrix_(),
      kkt_residual_(), s_(), d_(), riccati_factorization_(),
      solution_interpolator_(), solver_options_(), solver_statistics_(),
      timer_() {}
...
void OCPSolver::setSolverOptions(const SolverOptions &solver_options) {
  ...
  solution_interpolator_.setInterpolationOrder(
      solver_options.interpolation_order);
...
void OCPSolver::discretize(const double t) {
  ...
  if (solver_options_.discretization_method ==
      DiscretizationMethod::PhaseBased) {
    time_discretization_.correctTimeSteps(contact_sequence_, t);
  }
  resizeData();
  std::cout << "discretize cost" << std::endl;
  ocp_.cost->discretize(robots_[0], time_discretization_);
  std::cout << "done discretize cost" << std::endl;
}
...
void OCPSolver::updateSolution(const double t, const Eigen::VectorXd &q,
  ...
  dms_.evalKKT(robots_, time_discretization_, q, v, s_, kkt_matrix_,
               kkt_residual_);
  sto_.evalKKT(time_discretization_, kkt_matrix_, kkt_residual_);
  riccati_recursion_.backwardRiccatiRecursion(
      time_discretization_, kkt_matrix_, kkt_residual_, riccati_factorization_);
  dms_.computeInitialStateDirection(robots_[0], q, v, s_, d_);
  riccati_recursion_.forwardRiccatiRecursion(time_discretization_, kkt_matrix_,
                                             kkt_residual_,
  ...
  double primal_step_size =
      std::min(dms_.maxPrimalStepSize(), sto_.maxPrimalStepSize());
  const double dual_step_size =
      std::min(dms_.maxDualStepSize(), sto_.maxDualStepSize());
  if (solver_options_.enable_line_search) {
    const double max_primal_step_size = primal_step_size;
    primal_step_size =
        line_search_.computeStepSize(dms_, robots_, time_discretization_, q, v,
  ...
  dms_.integrateSolution(robots_, time_discretization_, primal_step_size,
                         dual_step_size, d_, s_);
  sto_.integrateSolution(time_discretization_, primal_step_size, dual_step_size,
                         d_);
}
...
void OCPSolver::solve(const double t, const Eigen::VectorXd &q,
  ...
  if (q.size() != robots_[0].dimq()) {
    throw std::out_of_range("[OCPSolver] invalid argument: q.size() must be " +
                            std::to_string(robots_[0].dimq()) + "!");
  }
  if (v.size() != robots_[0].dimv()) {
    throw std::out_of_range("[OCPSolver] invalid argument: v.size() must be " +
                            std::to_string(robots_[0].dimv()) + "!");
  }
  ...
  for (int iter = 0; iter < solver_options_.max_iter; ++iter, ++inner_iter) {
    ...
    if ((ocp_.sto_cost && ocp_.sto_constraints) &&
        (kkt_error < solver_options_.kkt_tol_mesh)) {
      if (time_discretization_.maxTimeStep() > solver_options_.max_dt_mesh) {
        ...
        discretize(t);
        if (solver_options_.enable_solution_interpolation) {
          solution_interpolator_.interpolate(robots_[0], time_discretization_,
                                             s_);
        }
        dms_.initConstraints(robots_, time_discretization_, s_);
        sto_.initConstraints(time_discretization_);
...
const Solution &OCPSolver::getSolution() const { return s_; }
...
OCPSolver::getSolution(const std::string &name,
  ...
  if (name == "q") {
    ...
    for (int i = 0; i < time_discretization_.size(); ++i) {
      if ((time_discretization_[i].type == GridType::Impact) ||
          (time_discretization_[i].type == GridType::Terminal)) {
        sol.push_back(Eigen::VectorXd::Zero(robot.max_dimf_contact()));
      } else {
        Eigen::VectorXd f(Eigen::VectorXd::Zero(robot.max_dimf_contact()));
        ...
        for (int j = 0; j < robot.maxNumContacts(); ++j) {
          if (s_[i].isContactActive(j)) {
            ...
            robot.transformFromLocalToWorld(
                contact_frame, s_[i].f_contact[j].template head<3>(),
    ...
    for (int i = 0; i < time_discretization_.size(); ++i) {
      if ((time_discretization_[i].type == GridType::Impact) ||
        ...
        for (int j = 0; j < robot.maxNumContacts(); ++j) {
          if (s_[i].isContactActive(j)) {
            f.template segment<3>(3 * j) =
                s_[i].f_contact[j].template head<3>();
          }
        }
        sol.push_back(f);
      }
    ...
    throw std::invalid_argument(
        "[OCPSolver] invalid arugment: name must be q, v, u, a, f!");
  }
  return sol;
}
...
void OCPSolver::setSolution(const Solution &s) { s_ = s; }
...
void OCPSolver::setSolution(const std::string &name,
  ...
  if (name == "q") {
    if (value.size() != robots_[0].dimq()) {
      throw std::out_of_range(
          "[OCPSolver] invalid argument: q.size() must be " +
          std::to_string(robots_[0].dimq()) + "!");
    }
    for (auto &e : s_) {
      e.q = value;
    ...
    if (value.size() != robots_[0].dimv()) {
      throw std::out_of_range(
          "[OCPSolver] invalid argument: v.size() must be " +
          std::to_string(robots_[0].dimv()) + "!");
    }
    for (auto &e : s_) {
      e.v = value;
    ...
    if ((value.size() != 3) && (value.size() != 6)) {
      throw std::out_of_range(
          "[OCPSolver] invalid argument: f.size() must be 3 or 6!");
    }
    for (int i = 0; i < time_discretization_.size(); ++i) {
      if (time_discretization_[i].type == GridType::Impact ||
          time_discretization_[i].type == GridType::Terminal)
        continue;
      if (value.size() == 3) {
        for (auto &ef : s_[i].f_contact) {
          ef.template head<3>() = value.template head<3>();
        }
      } else {
        for (auto &ef : s_[i].f_contact) {
    ...
    if ((value.size() != 3) && (value.size() != 6)) {
      throw std::out_of_range(
          "[OCPSolver] invalid argument: lmd.size() must be 3 or 6!");
    }
    for (int i = 0; i < time_discretization_.size(); ++i) {
      if (time_discretization_[i].type != GridType::Impact)
        continue;
    ...
    throw std::invalid_argument("[OCPSolver] invalid arugment: name must be "
                                "q, v, a, dv, u, f, or lmd!");
  }
}
  ...
  for (int i = 0; i < time_discretization_.size(); ++i) {
    ...
    if (grid.switching_constraint) {
      ...
      s_[i].setSwitchingConstraintDimension(
          contact_sequence_->impactStatus(grid_next_next.impact_index).dimf());
    } else {
...
void OCPSolver::disp(std::ostream &os) const { os << ocp_ << std::endl; }
