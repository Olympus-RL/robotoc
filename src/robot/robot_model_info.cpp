...
RobotModelInfo::RobotModelInfo(
    const std::string &_urdf_path, const BaseJointType _base_joint_type,
    const std::vector<ContactModelInfo> &_point_contacts,
    const std::vector<ContactModelInfo> &_surface_contacts,
    const std::vector<CKCInfo> &_ckcs, const double _contact_inv_damping)
    : urdf_path(_urdf_path), base_joint_type(_base_joint_type),
      point_contacts(_point_contacts), surface_contacts(_surface_contacts),
      ckcs(_ckcs), contact_inv_damping(_contact_inv_damping) {}
...
RobotModelInfo
RobotModelInfo::Quadruped(const std::string &urdf_path,
...
RobotModelInfo RobotModelInfo::Humanoid(
  ...
  return RobotModelInfo(urdf_path, BaseJointType::FloatingBase, {},
                        surface_contacts);
}
