...
PointContact::PointContact(const pinocchio::Model &model,
    ...
    : info_(info), contact_frame_id_(0), parent_joint_id_(0), dimv_(model.nv),
      jXf_(SE3::Identity()), J_frame_(Eigen::MatrixXd::Zero(6, model.nv)),
  ...
  if (!model.existFrame(info.frame)) {
    throw std::invalid_argument("[PointContact] invalid argument: frame '" +
                                info.frame + "' does not exit!");
  }
  ...
  if (info.baumgarte_position_gain < 0) {
    throw std::out_of_range("[PointContact] invalid argument: "
                            "'baumgarte_position_gain' must be non-negative!");
  }
  if (info.baumgarte_velocity_gain < 0) {
    throw std::out_of_range("[PointContact] invalid argument: "
                            "'baumgarte_velocity_gain' must be non-negative!");
  }
  v_frame_.setZero();
...
PointContact::PointContact()
    : info_(), contact_frame_id_(0), parent_joint_id_(0), dimv_(0),
      jXf_(SE3::Identity()), J_frame_(), frame_v_partial_dq_(),
...
PointContact::contactPosition(const pinocchio::Data &data) const {
  return data.oMf[contact_frame_id_].translation();
}

int PointContact::contactFrameId() const { return contact_frame_id_; }

int PointContact::parentJointId() const { return parent_joint_id_; }

const ContactModelInfo &PointContact::contactModelInfo() const { return info_; }
...
void PointContact::disp(std::ostream &os) const {
  ...
  os << "  Baumgarte's gains on (position, velocity): ("
     << info_.baumgarte_position_gain << ", " << info_.baumgarte_velocity_gain
     << ")" << std::flush;
}
