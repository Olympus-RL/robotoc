...
ContactWrenchCone::ContactWrenchCone(const Robot &robot, const double X,
                                     const double Y)
    : ConstraintComponentBase(), dimv_(robot.dimv()),
  ...
  if (robot.maxNumContacts() == 0) {
    throw std::out_of_range("[ContactWrenchCone] invalid argument: "
...
ContactWrenchCone::ContactWrenchCone()
    : ConstraintComponentBase(), dimc_(0), max_num_contacts_(0), dimv_(0),
...
void ContactWrenchCone::condenseSlackAndDual(
...
int ContactWrenchCone::dimc() const { return dimc_; }

void ContactWrenchCone::computeCone(const double mu,
...
void ContactWrenchCone::updateCone(const double mu,
                                   Eigen::MatrixXd &cone) const {
  for (int i = 1; i < 5; ++i) {
