...
ImpactWrenchCone::ImpactWrenchCone(const Robot &robot, const double X,
                                   const double Y)
    : ImpactConstraintComponentBase(),
      ...
      contact_types_(robot.contactTypes()), X_(X), Y_(Y) {
  if (robot.maxNumContacts() == 0) {
    throw std::out_of_range("[ImpactWrenchCone] invalid argument: "
                            "robot.maxNumContacts() must be positive!");
  }
  if (X <= 0) {
...
ImpactWrenchCone::ImpactWrenchCone()
    : ImpactConstraintComponentBase(), dimc_(0), max_num_contacts_(0), dimv_(0),
      contact_frame_(), contact_types_(), X_(0), Y_(0) {}
...
void ImpactWrenchCone::setSlack(Robot &robot, const ImpactStatus &impact_status,
                                ConstraintComponentData &data,
                                const SplitSolution &s) const {
  int c_begin = 0;
  for (int i = 0; i < max_num_contacts_; ++i) {
  ...
  for (int i = 0; i < max_num_contacts_; ++i) {
    ...
    case ContactType::SurfaceContact:
      if (impact_status.isImpactActive(i)) {
        ...
        updateCone(impact_status.frictionCoefficient(i), cone_i);
        data.residual.template segment<17>(c_begin).noalias() =
            cone_i * s.f_contact[i] + data.slack.template segment<17>(c_begin);
        computeComplementarySlackness<17>(data, c_begin);
        data.log_barrier +=
            logBarrier(data.slack.template segment<17>(c_begin));
      }
      c_begin += 17;
      break;
    default:
...
void ImpactWrenchCone::condenseSlackAndDual(
...
int ImpactWrenchCone::dimc() const { return dimc_; }

void ImpactWrenchCone::computeCone(const double mu,
                                   Eigen::MatrixXd &cone) const {
  const double XYmu = (X_ + Y_) * mu;
  cone.resize(17, 6);
  cone << 0, 0, -1, 0, 0, 0, -1, 0, -mu, 0, 0, 0, 1, 0, -mu, 0, 0, 0, 0, -1,
      -mu, 0, 0, 0, 0, 1, -mu, 0, 0, 0, 0, 0, -Y_, -1, 0, 0, 0, 0, -Y_, 1, 0, 0,
      0, 0, -X_, 0, -1, 0, 0, 0, -X_, 0, 1, 0, -Y_, -X_, -XYmu, mu, mu, -1, -Y_,
      X_, -XYmu, mu, -mu, -1, Y_, -X_, -XYmu, -mu, mu, -1, Y_, X_, -XYmu, -mu,
      -mu, -1, Y_, X_, -XYmu, mu, mu, 1, Y_, -X_, -XYmu, mu, -mu, 1, -Y_, X_,
      -XYmu, -mu, mu, 1, -Y_, -X_, -XYmu, -mu, -mu, 1;
}

void ImpactWrenchCone::updateCone(const double mu,
                                  Eigen::MatrixXd &cone) const {
  for (int i = 1; i < 5; ++i) {
