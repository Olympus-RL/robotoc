...
ImpactFrictionCone::ImpactFrictionCone(const Robot &robot)
    : ImpactConstraintComponentBase(), dimv_(robot.dimv()),
  ...
  if (robot.maxNumContacts() == 0) {
    throw std::out_of_range("[ImpactFrictionCone] invalid argument: "
...
void ImpactFrictionCone::allocateExtraData(
  ...
  for (int i = 0; i < max_num_contacts_; ++i) {
    ...
    cone_world << 0, 0, -1, 1, 0, 0, -1, 0, 0, 0, 1, 0, 0, -1, 0;
  ...
  for (int i = 0; i < max_num_contacts_; ++i) {
    if (impact_status.isImpactActive(i)) {
      ...
      robot.transformFromLocalToWorld(contact_frame_[i],
                                      s.f_contact[i].template head<3>(), fWi);
...
void ImpactFrictionCone::evalDerivatives(Robot &robot,
                                         const ImpactStatus &impact_status,
                                         ConstraintComponentData &data,
  ...
  for (int i = 0; i < max_num_contacts_; ++i) {
    if (impact_status.isImpactActive(i)) {
      ...
      for (int j = 0; j < 4; ++j) {
        cone_world_i.coeffRef(j + 1, 2) =
            -(impact_status.frictionCoefficient(i) / std::sqrt(2));
      }
      // Friction cone in the local frame of the contact surface.
      Eigen::MatrixXd &cone_local_i = cone_local(data, i);
      cone_local_i.noalias() =
          cone_world_i * impact_status.contactRotation(i).transpose();
      ...
      robot.getJacobianTransformFromLocalToWorld(contact_frame_[i], fWi,
                                                 dfWi_dq);
...
void ImpactFrictionCone::expandSlackAndDual(const ImpactStatus &impact_status,
                                            ConstraintComponentData &data,
...
int ImpactFrictionCone::dimc() const { return dimc_; }
