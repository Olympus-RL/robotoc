...
SplitSolution::SplitSolution(const Robot &robot)
    : q(Eigen::VectorXd::Zero(robot.dimq())),
      ...
      xi_stack_(Eigen::VectorXd::Zero(
          robot.max_dimf_contact())), // these are realted to switching
                                      // constraints
...
double SplitSolution::lagrangeMultiplierLinfNorm() const {
  ...
  const double mu_linf =
      ((dimf_ > 0) ? mu_stack().template lpNorm<Eigen::Infinity>() : 0);
  const double xi_linf =
      ((dims_ > 0) ? xi_stack().template lpNorm<Eigen::Infinity>() : 0);
  return std::max(
      {lmd_linf, gmm_linf, beta_linf, nu_passive_linf, mu_linf, xi_linf});
}
