...
SplitKKTMatrix::SplitKKTMatrix(const Robot &robot)
    : Fxx(Eigen::MatrixXd::Zero(2 * robot.dimv(), 2 * robot.dimv())),
      ...
      Quu(Eigen::MatrixXd::Zero(robot.dimu(), robot.dimu())), Qtt(0),
      Qtt_prev(0), hx(Eigen::VectorXd::Zero(2 * robot.dimv())),
      ha(Eigen::VectorXd::Zero(robot.dimv())),
      hu(Eigen::VectorXd::Zero(robot.dimu())),
      Phix_full_(
          Eigen::MatrixXd::Zero(robot.max_dimf_contact(), 2 * robot.dimv())),
      Phia_full_(Eigen::MatrixXd::Zero(robot.max_dimf_contact(), robot.dimv())),
      Phiu_full_(Eigen::MatrixXd::Zero(robot.max_dimf_contact(), robot.dimu())),
      Phit_full_(Eigen::VectorXd::Zero(robot.max_dimf_contact())),
      ...
      dimx_(2 * robot.dimv()), dimu_(robot.dimu()), dimf_(robot.dimf_ckc()),
      dimf_ckc_(robot.dimf_ckc()), dimf_contact_(0), dims_(0) {}

SplitKKTMatrix::SplitKKTMatrix()
    : Fxx(), Fvu(), fx(), Qxx(), Qaa(), Qdvdv(), Qxu(), Quu(), Qtt(0),
      Qtt_prev(0), hx(), ha(), hu(), Phix_full_(), Phia_full_(), Phiu_full_(),
      Phit_full_(), Qff_full_(), Qqf_full_(), hf_full_(),
      has_floating_base_(false), dimv_(0), dimx_(0), dimu_(0), dimf_(0),
      dims_(0) {}
...
bool SplitKKTMatrix::isApprox(const SplitKKTMatrix &other) const {
  if (!Fxx.isApprox(other.Fxx))
    return false;
  if (!Fvu.isApprox(other.Fvu))
    return false;
  if (!fx.isApprox(other.fx))
    return false;
  if (dims() > 0) {
    assert(dims() == other.dims());
    if (!Phix().isApprox(other.Phix()))
      return false;
    if (!Phia().isApprox(other.Phia()))
      return false;
    if (!Phiu().isApprox(other.Phiu()))
      return false;
    if (!Phit().isApprox(other.Phit()))
      return false;
  }
  if (!Qxx.isApprox(other.Qxx))
    return false;
  if (!Qaa.isApprox(other.Qaa))
    return false;
  if (!Qdvdv.isApprox(other.Qdvdv))
    return false;
  if (!Qxu.isApprox(other.Qxu))
    return false;
  if (!Quu.isApprox(other.Quu))
    return false;
  if (dimf() > 0) {
    assert(dimf() == other.dimf());
    if (!Qff().isApprox(other.Qff()))
      return false;
  ...
  if (!vec.isApprox(other_vec))
    return false;
  if (!hx.isApprox(other.hx))
    return false;
  if (!hu.isApprox(other.hu))
    return false;
  if (!ha.isApprox(other.ha))
    return false;
  if (!hf().isApprox(other.hf()))
    return false;
  return true;
}
...
void SplitKKTMatrix::setRandom() {
  ...
  Phiu().setRandom();
  Phit().setRandom();
  const Eigen::MatrixXd Qxxuu_seed =
      Eigen::MatrixXd::Random(dimx_ + dimu_, dimx_ + dimu_);
  ...
  const Eigen::MatrixXd Qaaff_seed =
      Eigen::MatrixXd::Random(dimv_ + dimf_, dimv_ + dimf_);
