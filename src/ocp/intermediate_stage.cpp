...
IntermediateStage::IntermediateStage(
    ...
    : cost_(cost), constraints_(constraints),
      contact_sequence_(contact_sequence) {}

IntermediateStage::IntermediateStage()
    : cost_(), constraints_(), contact_sequence_() {}
...
bool IntermediateStage::isFeasible(Robot &robot, const GridInfo &grid_info,
  ...
  assert(grid_info.type == GridType::Intermediate ||
         grid_info.type == GridType::Lift);
  const auto &contact_status =
      contact_sequence_->contactStatus(grid_info.phase);
  return constraints_->isFeasible(robot, contact_status, data.constraints_data,
                                  s);
}
...
void IntermediateStage::initConstraints(Robot &robot, const GridInfo &grid_info,
  ...
  data.constraints_data =
      constraints_->createConstraintsData(robot, grid_info.stage);
  const auto &contact_status =
      contact_sequence_->contactStatus(grid_info.phase);
  constraints_->setSlackAndDual(robot, contact_status, data.constraints_data,
                                s);
}
...
void IntermediateStage::evalOCP(Robot &robot, const GridInfo &grid_info,
  ...
  if (grid_info.switching_constraint) {
    const auto &impact_status =
        contact_sequence_->impactStatus(grid_info.impact_index + 1);
    evalSwitchingConstraint(robot, impact_status,
                            data.switching_constraint_data, grid_info.dt,
                            grid_info.dt_next, s, kkt_residual);
  }
...
void IntermediateStage::evalKKT(Robot &robot, const GridInfo &grid_info,
  ...
  constraints_->linearizeConstraints(robot, contact_status,
  ...
  linearizeContactDynamics(robot, contact_status, s, data.contact_dynamics_data,
                           kkt_residual);
  if (grid_info.switching_constraint) {
    const auto &impact_status =
        contact_sequence_->impactStatus(grid_info.impact_index + 1);
    linearizeSwitchingConstraint(
        robot, impact_status, data.switching_constraint_data, grid_info.dt,
        grid_info.dt_next, s, kkt_matrix, kkt_residual);
  } else {
  ...
  correctLinearizeStateEquation(robot, grid_info.dt, s, s_next,
                                data.state_equation_data, kkt_matrix,
                                kkt_residual);
  kkt_residual.h *= (1.0 / grid_info.num_grids_in_phase);
  kkt_matrix.hx.array() *= (1.0 / grid_info.num_grids_in_phase);
  kkt_matrix.hu.array() *= (1.0 / grid_info.num_grids_in_phase);
  kkt_matrix.fx.array() *= (1.0 / grid_info.num_grids_in_phase);
  kkt_matrix.Qtt *=
      (1.0 / (grid_info.num_grids_in_phase * grid_info.num_grids_in_phase));
...
void IntermediateStage::expandDual(const GridInfo &grid_info, OCPData &data,
  ...
  double dts = 0.0;
  if (grid_info.num_grids_in_phase > 0) {
    dts = (d.dts_next - d.dts) / grid_info.num_grids_in_phase;
  }
...
void computeInitialStateDirection(const Robot &robot, const Eigen::VectorXd &q0,
                                  const Eigen::VectorXd &v0,
                                  const SplitSolution &s0, const OCPData &data,
                                  SplitDirection &d0) {
  computeInitialStateDirection(robot, q0, v0, s0, data.state_equation_data, d0);
}
