...
DerivativeChecker::DerivativeChecker(const Robot &robot,
    ...
    : robot_(robot), finite_diff_(finite_diff), test_tol_(test_tol) {}
...
bool DerivativeChecker::checkFirstOrderStageCostDerivatives(
  ...
  return checkFirstOrderStageCostDerivatives(cost,
                                             robot_.createContactStatus());
}

bool DerivativeChecker::checkSecondOrderStageCostDerivatives(
    const std::shared_ptr<CostFunctionComponentBase> &cost) {
  return checkSecondOrderStageCostDerivatives(cost,
                                              robot_.createContactStatus());
}
...
bool DerivativeChecker::checkFirstOrderStageCostDerivatives(
  ...
  cost->evalStageCostDerivatives(robot_, contact_status, data, grid_info, s,
                                 kkt_residual);
  auto s1 = s;
...
bool DerivativeChecker::checkSecondOrderStageCostDerivatives(
  ...
  kkt_residual0.setContactDimension(contact_status.dimf());
  ...
  cost->evalStageCostDerivatives(robot_, contact_status, data, grid_info, s,
                                 kkt_residual0);
  cost->evalStageCostHessian(robot_, contact_status, data, grid_info, s,
                             kkt_matrix);
  SplitKKTResidual kkt_residual(robot_);
  kkt_residual.setContactDimension(contact_status.dimf());
  auto s1 = s;
  ...
  for (int i = 0; i < dimv; ++i) {
    ...
    cost->evalStageCost(robot_, contact_status, data, grid_info, s1);
    cost->evalStageCostDerivatives(robot_, contact_status, data, grid_info, s1,
                                   kkt_residual);
  ...
  if (dimf > 0) {
    for (int i = 0; i < dimf; ++i) {
      ...
      cost->evalStageCost(robot_, contact_status, data, grid_info, s1);
      cost->evalStageCostDerivatives(robot_, contact_status, data, grid_info,
                                     s1, kkt_residual);
...
bool DerivativeChecker::checkFirstOrderTerminalCostDerivatives(
  ...
  for (int i = 0; i < dimv; ++i) {
    ...
    lq_ref(i) = (cost->evalTerminalCost(robot_, data, grid_info, s1) - cost0) /
                finite_diff_;
  }
  if (!kkt_residual.lq().isApprox(lq_ref, test_tol_)) {
    std::cout << "lq is not correct! lq - lq_ref = "
  ...
  for (int i = 0; i < dimv; ++i) {
    ...
    s1.v(i) += finite_diff_;
    robot_.updateKinematics(s1.q, s1.v);
    lv_ref(i) = (cost->evalTerminalCost(robot_, data, grid_info, s1) - cost0) /
                finite_diff_;
  }
  if (!kkt_residual.lv().isApprox(lv_ref, test_tol_)) {
    std::cout << "lv is not correct! lv - lv_ref = "
...
bool DerivativeChecker::checkSecondOrderTerminalCostDerivatives(
  ...
  for (int i = 0; i < dimv; ++i) {
    ...
    cost->evalTerminalCostDerivatives(robot_, data, grid_info, s1,
                                      kkt_residual);
    Qqq_ref.col(i) = (kkt_residual.lq() - kkt_residual0.lq()) / finite_diff_;
  }
  if (!kkt_matrix.Qqq().isApprox(Qqq_ref, test_tol_)) {
...
bool DerivativeChecker::checkFirstOrderImpactCostDerivatives(
    const std::shared_ptr<CostFunctionComponentBase> &cost) {
  return checkFirstOrderImpactCostDerivatives(cost,
                                              robot_.createImpactStatus());
}

bool DerivativeChecker::checkSecondOrderImpactCostDerivatives(
    const std::shared_ptr<CostFunctionComponentBase> &cost) {
  return checkSecondOrderImpactCostDerivatives(cost,
                                               robot_.createImpactStatus());
}
...
bool DerivativeChecker::checkFirstOrderImpactCostDerivatives(
  ...
  const auto s = SplitSolution::Random(robot_, impact_status);
  const auto grid_info = GridInfo::Random();
  const int dimv = robot_.dimv();
  ...
  robot_.updateKinematics(s.q, s.v);
  double cost0 =
      cost->evalImpactCost(robot_, impact_status, data, grid_info, s);
  cost->evalImpactCostDerivatives(robot_, impact_status, data, grid_info, s,
                                  kkt_residual);
  auto s1 = s;
  Eigen::VectorXd lq_ref(dimv);
  for (int i = 0; i < dimv; ++i) {
    ...
    robot_.updateKinematics(s1.q, s1.v);
    lq_ref(i) =
        (cost->evalImpactCost(robot_, impact_status, data, grid_info, s1) -
         cost0) /
        finite_diff_;
  }
  if (!kkt_residual.lq().isApprox(lq_ref, test_tol_)) {
...
bool DerivativeChecker::checkSecondOrderImpactCostDerivatives(
  ...
  SplitKKTResidual kkt_residual0(robot_);
  kkt_residual0.setContactDimension(impact_status.dimf());
  CostFunctionData data(robot_);
  robot_.updateKinematics(s.q, s.v);
  cost->evalImpactCost(robot_, impact_status, data, grid_info, s);
  cost->evalImpactCostDerivatives(robot_, impact_status, data, grid_info, s,
                                  kkt_residual0);
  cost->evalImpactCostHessian(robot_, impact_status, data, grid_info, s,
                              kkt_matrix);
  SplitKKTResidual kkt_residual(robot_);
  kkt_residual.setContactDimension(impact_status.dimf());
  auto s1 = s;
  ...
  for (int i = 0; i < dimv; ++i) {
    ...
    robot_.updateKinematics(s1.q, s1.v);
    cost->evalImpactCost(robot_, impact_status, data, grid_info, s1);
    cost->evalImpactCostDerivatives(robot_, impact_status, data, grid_info, s1,
                                    kkt_residual);
    Qqq_ref.col(i) = (kkt_residual.lq() - kkt_residual0.lq()) / finite_diff_;
  }
  if (!kkt_matrix.Qqq().isApprox(Qqq_ref, test_tol_)) {
  ...
  if (dimf > 0) {
    for (int i = 0; i < dimf; ++i) {
      ...
      kkt_residual.lf().setZero();
      cost->evalImpactCost(robot_, impact_status, data, grid_info, s1);
      cost->evalImpactCostDerivatives(robot_, impact_status, data, grid_info,
                                      s1, kkt_residual);
      Qff_ref.col(i) = (kkt_residual.lf() - kkt_residual0.lf()) / finite_diff_;
    }
    if (!kkt_matrix.Qff().isApprox(Qff_ref, test_tol_)) {
