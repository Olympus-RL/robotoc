...
ContactDynamicsData::ContactDynamicsData(const Robot &robot)
    : Qxu_passive(Eigen::MatrixXd::Zero(2 * robot.dimv(), robot.dim_passive())),
      Quu_passive_topRight(
          Eigen::MatrixXd::Zero(robot.dim_passive(), robot.dimu())),
      ...
      MJtJinv_dIDCdqv_full_(Eigen::MatrixXd::Zero(
          robot.dimv() + robot.max_dimf(), 2 * robot.dimv())),
...
ContactDynamicsData::ContactDynamicsData()
    : Qxu_passive(), Quu_passive_topRight(), lu_passive(), dIDda(), dIDddv(),
      dIDCdqv_full_(), MJtJinv_full_(), MJtJinv_dIDCdqv_full_(), Qafqv_full_(),
      Qafu_full_full_(), IDC_full_(), MJtJinv_IDC_full_(), Phia_full_(),
      laf_full_(), haf_full_(), dimv_(0), dimu_(0), dimf_(0), dimvf_(0),
      dimf_contact_(0), dimf_ckc_(0), dims_(0), dim_passive_(0),
