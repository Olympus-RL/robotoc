...
void linearizeImpactDynamics(Robot &robot, const ImpactStatus &impact_status,
  ...
  kkt_residual.lq().noalias() += data.dCdq().transpose() * s.rhomu_stack();
  kkt_residual.lv().noalias() += data.dCdv().transpose() * s.rhomu_stack();
  kkt_residual.ldv.noalias() += data.dCdv().transpose() * s.rhomu_stack();
...
void expandImpactDynamicsPrimal(const ContactDynamicsData &data,
                                SplitDirection &d) {
  d.ddvgf().noalias() = -data.MJtJinv_dIDCdqv() * d.dx;
  d.ddvgf().noalias() -= data.MJtJinv_IDC();
  d.dgf().array() *= -1;
}
...
void expandImpactDynamicsDual(ContactDynamicsData &data,
  ...
  d.dbetarhomu().noalias() = -data.MJtJinv() * data.ldvf();
}
